...
fn main() {
    ...
    // load font.txt
    let input = fs::read_to_string("font.txt")
        .expect("Failed to read font.txt");
    ...
    while i < lines.len() {
        ...
        let char_code_str = line
            .trim_start_matches("0x")
            .trim_start_matches("0X");
        if let Ok(char_code) =
            usize::from_str_radix(char_code_str, 16)
        {
            if char_code >= 256 {
                break;
            }
            ...
            for (y, bitmap_line) in
                lines.iter().skip(i).take(16).enumerate()
            {
                let row_chars: Vec<char> =
                    bitmap_line.chars().collect();
...
fn generate_rust_code(
    ...
    fs::write(dest_path, output)
        .expect("Failed to write font_table.rs");
}
