...
impl<const LEVEL: usize, const SHIFT: usize, NEXT> Entry<LEVEL, SHIFT, NEXT> {
    ...
    fn populate(&mut self) -> Result<&mut Self> {
        if self.is_present() {
            Err("Entry is already populated")
        } else {
            let next: Box<NEXT> =
...
pub unsafe fn write_cs(cs: u16) {
    // The MOV instruction CANNOT be used to load the CS
    // register. Use far-jump(ljmp) instead.
    unsafe {
        asm!(
...
extern "sysv64" fn inthandler(info: &InterruptInfo, index: usize) {
    ...
    match index {
        ...
        13 => {
            error!("General Protextion Fault");
        ...
        14 => {
            ...
            error!(
                "Caused by: A {} mode {} on a {} page, page structure are {}",
...
pub const BIT_FLAG_INTGATE: u8 = 0b0000_1110u8;
pub const BIT_FLAG_PRESENT: u8 = 0b1000_0000u8;
pub const BIT_FLAG_DPL0: u8 = 0 << 5;
pub const BIT_FLAG_DPL3: u8 = 3 << 5;
...
enum IdtAttr {
    ...
    _NotPresent = 0,
    IntGateDPL0 = BIT_FLAG_INTGATE | BIT_FLAG_PRESENT | BIT_FLAG_DPL0,
    IntGateDPL3 = BIT_FLAG_INTGATE | BIT_FLAG_PRESENT | BIT_FLAG_DPL3,
}
...
impl TaskStateSegment64Descriptor {
    const fn new(base_addr: u64) -> Self {
        Self {
            ...
            base_high: ((base_addr >> 32) & 0xffff_ffff) as u32,
            reserved: 0,
        }
    }
}
...
/// # Safety
/// Writing to CR3 can causes any exceptions so it is programmer's
/// responsibility to setup correct page tables.
#[unsafe(no_mangle)]
